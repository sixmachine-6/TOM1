...
function angularvelocity(){
    ...
    const outputangle=parseInt(2*((180*(Math.atan(X)))/Math.PI));
    console.log( outputangle);
    const Y=l4*(Math.sin(inputangle-outputangle))+l1*(Math.sin(inputangle));
    console.log(Y);
    const Z=l2*(Math.sin(inputangle-outputangle))+l1*(Math.sin(outputangle));
    console.log(Z);
    const angular=parseInt(((l2*Y)/(l4*Z))*input6.value);
    console.log(angular);
    para2.innerText=`the angular velocity is ${angular} cm/s`;
    }
}